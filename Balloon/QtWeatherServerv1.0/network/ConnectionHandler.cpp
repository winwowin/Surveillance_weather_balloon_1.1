...
void ConnectionHandler::threadLoop(){
    ...
    //this is for the manual release
    if(rec=="manualRelease"){
        digitalWrite(0,HIGH);           //writes to servo to rotate to release position
        cout<<"servo actuated"<<endl;
    }


    //this is for forcing an update of sensor readings from the client user
    if (rec == "getReading"){
    ...
    //this is for forcing a photo to be taken from the client user
    //it will take and label an image incrimentally up to 999
    // the "-n" flag is used to disable preview, as that would be a waste of system resources
    if (rec =="getPicture"){
        cout<<"recieved from the client ["<<rec<<"]"<<end;
        system("raspistill -t 100 -o userCapture_%03d.jpg -n");
